...
// 获取某一行的最后一个非空白字符
char ScintillaEditBase::GetLastNonWhitespaceChar(int line) {
    // 获取行的起始和结束位置
    int lineStart = send(SCI_POSITIONFROMLINE, line, 0);
    int lineEnd = send(SCI_GETLINEENDPOSITION, line, 0);

    // 从行尾向前遍历
    for (int pos = lineEnd - 1; pos >= lineStart; --pos) {
        char ch = static_cast<char>(send(SCI_GETCHARAT, pos, 0));
        // 检查是否是非空白字符
        if (!isspace(ch)) {
            return ch; // 找到最后一个非空白字符
        }
    }
...
void ScintillaEditBase::notifyParent(NotificationData scn)
{
   ...
   //                 ));
            ...
            //    send(SCI_INSERTTEXT, send(SCI_GETCURRENTPOS, 0, 0), reinterpret_cast<sptr_t>(rightBracket.c_str()));
            //    send(SCI_GOTOPOS, send(SCI_GETCURRENTPOS, 0, 0), 0);
            //}
            {
                //输入分括号改变缩进

                // 获取当前光标位置
                int currentPos = send(SCI_GETCURRENTPOS, 0, 0);
                int line = send(SCI_LINEFROMPOSITION, currentPos, 0);
                if (scn.ch == '\n')
                {
                    // 获取上一行的缩进
                    int indent = send(SCI_GETLINEINDENTATION, line - 1, 0);
                    // 如果上一行以左括号结尾，则增加缩进
                    char prevChar = GetLastNonWhitespaceChar(line - 1);
                    int currline_indent = 0;
                    if (prevChar == '{') {
                        // 增加缩进
                        currline_indent = indent + 4;
                        send(SCI_SETLINEINDENTATION, line, currline_indent);
                        send(SCI_GOTOPOS, currentPos + currline_indent, 0);
                    }
                    else {
                        currline_indent = indent;
                        send(SCI_SETLINEINDENTATION, line, currline_indent);
                        send(SCI_GOTOPOS, currentPos + currline_indent, 0);
                    }
                }
                if (scn.ch == '}') {
                    // 获取当前行的缩进
                    int indent = send(SCI_GETLINEINDENTATION, line, 0);
                    send(SCI_SETLINEINDENTATION, line, indent - 4);
                }
            }
